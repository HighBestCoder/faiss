...
void fvec_inner_product_batch_4(
...
#ifdef __AVX512F__
    size_t i = 0;
    const size_t d16 = d / 16 * 16;
    __m512 d0v = _mm512_setzero_ps();
    __m512 d1v = _mm512_setzero_ps();
    __m512 d2v = _mm512_setzero_ps();
    __m512 d3v = _mm512_setzero_ps();

    for (; i < d16; i += 16) {
        const __m512 xv = _mm512_loadu_ps(x + i);
        d0v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y0 + i), d0v);
        d1v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y1 + i), d1v);
        d2v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y2 + i), d2v);
        d3v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y3 + i), d3v);
    }

    float d0 = _mm512_reduce_add_ps(d0v);
    float d1 = _mm512_reduce_add_ps(d1v);
    float d2 = _mm512_reduce_add_ps(d2v);
    float d3 = _mm512_reduce_add_ps(d3v);
    ...
    __m256 d0v = _mm256_setzero_ps();
    __m256 d1v = _mm256_setzero_ps();
    __m256 d2v = _mm256_setzero_ps();
    __m256 d3v = _mm256_setzero_ps();

    for (; i < d8; i += 8) {
        const __m256 xv = _mm256_loadu_ps(x + i);
        d0v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y0 + i), d0v);
        d1v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y1 + i), d1v);
        d2v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y2 + i), d2v);
        d3v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y3 + i), d3v);
    }

    const __m128 d0v_lo = _mm256_castps256_ps128(d0v);
    const __m128 d0v_hi = _mm256_extractf128_ps(d0v, 1);
    __m128 sum0 = _mm_add_ps(d0v_lo, d0v_hi);
    __m128 shuf0 = _mm_shuffle_ps(sum0, sum0, _MM_SHUFFLE(0, 0, 3, 2));
    sum0 = _mm_add_ps(sum0, shuf0);
    shuf0 = _mm_shuffle_ps(sum0, sum0, _MM_SHUFFLE(0, 0, 0, 1));
    sum0 = _mm_add_ps(sum0, shuf0);
    float d0 = _mm_cvtss_f32(sum0);

    const __m128 d1v_lo = _mm256_castps256_ps128(d1v);
    const __m128 d1v_hi = _mm256_extractf128_ps(d1v, 1);
    __m128 sum1 = _mm_add_ps(d1v_lo, d1v_hi);
    __m128 shuf1 = _mm_shuffle_ps(sum1, sum1, _MM_SHUFFLE(0, 0, 3, 2));
    sum1 = _mm_add_ps(sum1, shuf1);
    shuf1 = _mm_shuffle_ps(sum1, sum1, _MM_SHUFFLE(0, 0, 0, 1));
    sum1 = _mm_add_ps(sum1, shuf1);
    float d1 = _mm_cvtss_f32(sum1);

    const __m128 d2v_lo = _mm256_castps256_ps128(d2v);
    const __m128 d2v_hi = _mm256_extractf128_ps(d2v, 1);
    __m128 sum2 = _mm_add_ps(d2v_lo, d2v_hi);
    __m128 shuf2 = _mm_shuffle_ps(sum2, sum2, _MM_SHUFFLE(0, 0, 3, 2));
    sum2 = _mm_add_ps(sum2, shuf2);
    shuf2 = _mm_shuffle_ps(sum2, sum2, _MM_SHUFFLE(0, 0, 0, 1));
    sum2 = _mm_add_ps(sum2, shuf2);
    float d2 = _mm_cvtss_f32(sum2);

    const __m128 d3v_lo = _mm256_castps256_ps128(d3v);
    const __m128 d3v_hi = _mm256_extractf128_ps(d3v, 1);
    __m128 sum3 = _mm_add_ps(d3v_lo, d3v_hi);
    __m128 shuf3 = _mm_shuffle_ps(sum3, sum3, _MM_SHUFFLE(0, 0, 3, 2));
    sum3 = _mm_add_ps(sum3, shuf3);
    shuf3 = _mm_shuffle_ps(sum3, sum3, _MM_SHUFFLE(0, 0, 0, 1));
    sum3 = _mm_add_ps(sum3, shuf3);
    float d3 = _mm_cvtss_f32(sum3);
...
void fvec_L2sqr_batch_4(
    ...
    for (; i < d16; i += 16) {
        const __m512 xv = _mm512_loadu_ps(x + i);
        const __m512 q0 = _mm512_sub_ps(xv, _mm512_loadu_ps(y0 + i));
        const __m512 q1 = _mm512_sub_ps(xv, _mm512_loadu_ps(y1 + i));
        const __m512 q2 = _mm512_sub_ps(xv, _mm512_loadu_ps(y2 + i));
        const __m512 q3 = _mm512_sub_ps(xv, _mm512_loadu_ps(y3 + i));
        d0v = _mm512_fmadd_ps(q0, q0, d0v);
        d1v = _mm512_fmadd_ps(q1, q1, d1v);
        d2v = _mm512_fmadd_ps(q2, q2, d2v);
        d3v = _mm512_fmadd_ps(q3, q3, d3v);
    }
    ...
    for (; i < d8; i += 8) {
        const __m256 xv = _mm256_loadu_ps(x + i);
        const __m256 q0 = _mm256_sub_ps(xv, _mm256_loadu_ps(y0 + i));
        const __m256 q1 = _mm256_sub_ps(xv, _mm256_loadu_ps(y1 + i));
        const __m256 q2 = _mm256_sub_ps(xv, _mm256_loadu_ps(y2 + i));
        const __m256 q3 = _mm256_sub_ps(xv, _mm256_loadu_ps(y3 + i));
        d0v = _mm256_fmadd_ps(q0, q0, d0v);
        d1v = _mm256_fmadd_ps(q1, q1, d1v);
        d2v = _mm256_fmadd_ps(q2, q2, d2v);
        d3v = _mm256_fmadd_ps(q3, q3, d3v);
    }
...
void fvec_inner_product_batch_8(
    ...
    __m512 d2v = _mm512_setzero_ps();
    __m512 d3v = _mm512_setzero_ps();
    __m512 d4v = _mm512_setzero_ps();
    __m512 d5v = _mm512_setzero_ps();
    __m512 d6v = _mm512_setzero_ps();
    __m512 d7v = _mm512_setzero_ps();
    ...
    for (; i < d16; i += 16) {
        ...
        d2v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y2 + i), d2v);
        d3v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y3 + i), d3v);
        d4v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y4 + i), d4v);
        d5v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y5 + i), d5v);
        d6v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y6 + i), d6v);
        d7v = _mm512_fmadd_ps(xv, _mm512_loadu_ps(y7 + i), d7v);
    }
    ...
    float d2 = _mm512_reduce_add_ps(d2v);
    float d3 = _mm512_reduce_add_ps(d3v);
    float d4 = _mm512_reduce_add_ps(d4v);
    float d5 = _mm512_reduce_add_ps(d5v);
    float d6 = _mm512_reduce_add_ps(d6v);
    float d7 = _mm512_reduce_add_ps(d7v);
    ...
    dis7 = d7;
#elif defined(__AVX2__)
    auto horizontal_sum_256 = [](__m256 v) -> float {
        const __m128 v_lo = _mm256_castps256_ps128(v);
        const __m128 v_hi = _mm256_extractf128_ps(v, 1);
        __m128 sum = _mm_add_ps(v_lo, v_hi);
        __m128 shuf = _mm_shuffle_ps(sum, sum, _MM_SHUFFLE(0, 0, 3, 2));
        sum = _mm_add_ps(sum, shuf);
        shuf = _mm_shuffle_ps(sum, sum, _MM_SHUFFLE(0, 0, 0, 1));
        sum = _mm_add_ps(sum, shuf);
    ...
    __m256 d2v = _mm256_setzero_ps();
    __m256 d3v = _mm256_setzero_ps();
    __m256 d4v = _mm256_setzero_ps();
    __m256 d5v = _mm256_setzero_ps();
    __m256 d6v = _mm256_setzero_ps();
    __m256 d7v = _mm256_setzero_ps();
    ...
    for (; i < d8; i += 8) {
        ...
        d2v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y2 + i), d2v);
        d3v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y3 + i), d3v);
        d4v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y4 + i), d4v);
        d5v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y5 + i), d5v);
        d6v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y6 + i), d6v);
        d7v = _mm256_fmadd_ps(xv, _mm256_loadu_ps(y7 + i), d7v);
    }

    float d0 = horizontal_sum_256(d0v);
    float d1 = horizontal_sum_256(d1v);
    float d2 = horizontal_sum_256(d2v);
    float d3 = horizontal_sum_256(d3v);
    float d4 = horizontal_sum_256(d4v);
    float d5 = horizontal_sum_256(d5v);
    float d6 = horizontal_sum_256(d6v);
    float d7 = horizontal_sum_256(d7v);
...
void fvec_L2sqr_batch_8(
    ...
    for (; i < d16; i += 16) {
        ...
        const __m512 q2 = _mm512_sub_ps(xv, _mm512_loadu_ps(y2 + i));
        const __m512 q3 = _mm512_sub_ps(xv, _mm512_loadu_ps(y3 + i));
        const __m512 q4 = _mm512_sub_ps(xv, _mm512_loadu_ps(y4 + i));
        const __m512 q5 = _mm512_sub_ps(xv, _mm512_loadu_ps(y5 + i));
        const __m512 q6 = _mm512_sub_ps(xv, _mm512_loadu_ps(y6 + i));
        const __m512 q7 = _mm512_sub_ps(xv, _mm512_loadu_ps(y7 + i));
        d0v = _mm512_fmadd_ps(q0, q0, d0v);
        d1v = _mm512_fmadd_ps(q1, q1, d1v);
        d2v = _mm512_fmadd_ps(q2, q2, d2v);
        d3v = _mm512_fmadd_ps(q3, q3, d3v);
        d4v = _mm512_fmadd_ps(q4, q4, d4v);
        d5v = _mm512_fmadd_ps(q5, q5, d5v);
        d6v = _mm512_fmadd_ps(q6, q6, d6v);
        d7v = _mm512_fmadd_ps(q7, q7, d7v);
    }
    ...
    for (; i < d8; i += 8) {
        ...
        const __m256 q2 = _mm256_sub_ps(xv, _mm256_loadu_ps(y2 + i));
        const __m256 q3 = _mm256_sub_ps(xv, _mm256_loadu_ps(y3 + i));
        const __m256 q4 = _mm256_sub_ps(xv, _mm256_loadu_ps(y4 + i));
        const __m256 q5 = _mm256_sub_ps(xv, _mm256_loadu_ps(y5 + i));
        const __m256 q6 = _mm256_sub_ps(xv, _mm256_loadu_ps(y6 + i));
        const __m256 q7 = _mm256_sub_ps(xv, _mm256_loadu_ps(y7 + i));
        d0v = _mm256_fmadd_ps(q0, q0, d0v);
        d1v = _mm256_fmadd_ps(q1, q1, d1v);
        d2v = _mm256_fmadd_ps(q2, q2, d2v);
        d3v = _mm256_fmadd_ps(q3, q3, d3v);
        d4v = _mm256_fmadd_ps(q4, q4, d4v);
        d5v = _mm256_fmadd_ps(q5, q5, d5v);
        d6v = _mm256_fmadd_ps(q6, q6, d6v);
        d7v = _mm256_fmadd_ps(q7, q7, d7v);
    }
